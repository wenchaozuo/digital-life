...
fn open_after_mutex<G: StorageUpgradeGate>(
    ...
    if version == connection::MAX_SUPPORTED_SCHEMA_VERSION {
        ...
        migration::validate_life_intent_schema(&connection)?;
        ...
        migration::validate_capability_authorization_schema(&connection)?;
    ...
    if upgraded_version == migration::EXPERIENCE_EPISODE_SCHEMA_VERSION {
        let life_intent_upgrade = migration::apply_life_intent_schema_upgrade(&transaction)?;
        if life_intent_upgrade != migration::LifeIntentAuthoritySchemaUpgrade::Applied {
            return Err(StorageError::migration_version_invariant_failed());
        }
        upgraded_version = migration::LIFE_INTENT_AUTHORITY_SCHEMA_VERSION;
    }
    if upgraded_version == migration::LIFE_INTENT_AUTHORITY_SCHEMA_VERSION {
        let autonomy_upgrade = migration::apply_autonomy_schema_upgrade(&transaction)?;
        if autonomy_upgrade != migration::AutonomyAuthoritySchemaUpgrade::Applied {
            return Err(StorageError::migration_version_invariant_failed());
        }
        upgraded_version = migration::AUTONOMY_AUTHORITY_SCHEMA_VERSION;
    }
    if upgraded_version == migration::AUTONOMY_AUTHORITY_SCHEMA_VERSION {
        let perception_upgrade = migration::apply_perception_schema_upgrade(&transaction)?;
        if perception_upgrade != migration::PerceptionAuthoritySchemaUpgrade::Applied {
            return Err(StorageError::migration_version_invariant_failed());
        }
        upgraded_version = migration::PERCEPTION_AUTHORITY_SCHEMA_VERSION;
    }
    if upgraded_version == migration::PERCEPTION_AUTHORITY_SCHEMA_VERSION {
        let body_package_upgrade = migration::apply_body_package_schema_upgrade(&transaction)?;
        if body_package_upgrade != migration::BodyPackageAuthoritySchemaUpgrade::Applied {
            return Err(StorageError::migration_version_invariant_failed());
        }
    ...
    if upgraded_version == migration::LIVE2D_CORE_AUTHORITY_SCHEMA_VERSION {
        let screen_perception_upgrade =
            migration::apply_screen_perception_schema_upgrade(&transaction)?;
        if screen_perception_upgrade != migration::ScreenPerceptionAuthoritySchemaUpgrade::Applied {
            return Err(StorageError::migration_version_invariant_failed());
        }
    ...
    if upgraded_version == migration::VISION_MODEL_PROFILE_SCHEMA_VERSION {
        let capability_authorization_upgrade =
            migration::apply_capability_authorization_schema_upgrade(&transaction)?;
        if capability_authorization_upgrade
            != migration::CapabilityAuthorizationSchemaUpgrade::Applied
        {
            return Err(StorageError::migration_version_invariant_failed());
        }
        upgraded_version = migration::CAPABILITY_AUTHORIZATION_SCHEMA_VERSION;
    }
...
mod tests {
    ...
    fn prepare_schema_version(path: &Path, version: i64) {
        assert!(
            version == migration::LAST_STATIC_MIGRATION_VERSION
                ...
                || version == migration::LIFE_INTENT_AUTHORITY_SCHEMA_VERSION
                ...
                || version == migration::VISION_MODEL_PROFILE_SCHEMA_VERSION
                || version == migration::CAPABILITY_AUTHORIZATION_SCHEMA_VERSION
        ...
        if version >= migration::LIFE_INTENT_AUTHORITY_SCHEMA_VERSION {
            assert_eq!(
                migration::apply_life_intent_schema_upgrade(&transaction).unwrap(),
                migration::LifeIntentAuthoritySchemaUpgrade::Applied
            );
        }
        if version >= migration::AUTONOMY_AUTHORITY_SCHEMA_VERSION {
            assert_eq!(
                migration::apply_autonomy_schema_upgrade(&transaction).unwrap(),
                migration::AutonomyAuthoritySchemaUpgrade::Applied
            );
        }
        if version >= migration::PERCEPTION_AUTHORITY_SCHEMA_VERSION {
            assert_eq!(
                migration::apply_perception_schema_upgrade(&transaction).unwrap(),
                migration::PerceptionAuthoritySchemaUpgrade::Applied
            );
        }
        if version >= migration::BODY_PACKAGE_AUTHORITY_SCHEMA_VERSION {
            assert_eq!(
        ...
        if version >= migration::SCREEN_PERCEPTION_AUTHORITY_SCHEMA_VERSION {
            assert_eq!(
        ...
        if version >= migration::CAPABILITY_AUTHORIZATION_SCHEMA_VERSION {
            assert_eq!(
                migration::apply_capability_authorization_schema_upgrade(&transaction).unwrap(),
                migration::CapabilityAuthorizationSchemaUpgrade::Applied
            );
        }
    ...
    #[test]
    fn storage_upgrade_coordinator_applies_schema_030_once_and_reopens_as_current() {
        let (_root, path) = database_path("schema-thirty-upgrade");
        prepare_schema_version(&path, migration::VISION_MODEL_PROFILE_SCHEMA_VERSION);

        let first = open_coordinated_storage_connection(&path).unwrap();
        assert_eq!(
            connection::read_schema_version(&first).unwrap(),
            connection::MAX_SUPPORTED_SCHEMA_VERSION
        );
        assert_eq!(
            first
                .query_row(
                    ...
                    [migration::CAPABILITY_AUTHORIZATION_SCHEMA_VERSION],
                    |row| row.get::<_, i64>(0),
                )
                .unwrap(),
            1
            ...
            first
                .query_row(
                    "SELECT COUNT(*) FROM sqlite_schema
                     WHERE type='trigger' AND name GLOB 'digital_life_writer_epoch_life_capability_authorization*'",
                    [],
                    |row| row.get::<_, i64>(0),
                )
                .unwrap(),
        ...
        let second = open_coordinated_storage_connection(&path).unwrap();
        assert_eq!(
            connection::read_schema_version(&second).unwrap(),
            connection::MAX_SUPPORTED_SCHEMA_VERSION
        );
        assert_eq!(
            second
    ...
    #[test]
    fn d26_a_coordinator_upgrades_schema_twenty_seven_to_schema_thirty_without_vision_backfill() {
    ...
    fn storage_initialize_restored_version_fourteen_validates_then_publishes() {
        ...
        // The schema is at 30, so the version-17 lifecycle validator cannot
        // run alone; the full 120-trigger writer fence plus the exact Schema-19
        ...
        assert_eq!(writer_fence_count(&state.connection), 120);
        assert_eq!(
            take_upgrade_events(),
            vec![
                "mutex",
    ...
    #[test]
    fn d14_b1_coordinator_upgrades_schema_twenty_one_without_goal_plan_action_backfill() {
        let _ = take_upgrade_events();
        let (_root, path) = database_path("schema-twenty-one-life-intent-upgrade");
        prepare_schema_version(&path, migration::EXPERIENCE_EPISODE_SCHEMA_VERSION);
        {
            let authorized = connection::open_authorized_test_connection(&path).unwrap();
            authorized
                .execute_batch(
                    "INSERT INTO persona_template (id, name, version, persona_json)
                     VALUES ('persona-d14', 'Persona', 1, '{}');
                     INSERT INTO life_identity
                         (id, name, created_at, version, body_id, persona_id, persona_version)
                     VALUES ('life-d14', 'Life D14', '2026-08-27T00:00:00.000Z', 1,
                             'body-d14', 'persona-d14', 1);
                     INSERT INTO conversation
                         (id, life_id, title, revision, created_at, updated_at, last_message_at)
                     VALUES ('conversation-d14', 'life-d14', 'D14 Conversation', 1,
                             '2026-08-27T00:00:00.000Z', '2026-08-27T00:00:00.002Z',
                             '2026-08-27T00:00:00.002Z');
                     INSERT INTO conversation_message
                         (id, conversation_id, life_id, turn_id, role, content, sequence_no, created_at)
                     VALUES
                         ('user-d14', 'conversation-d14', 'life-d14', 'turn-d14',
                          'user', 'historical user content', 1, '2026-08-27T00:00:00.001Z'),
                         ('assistant-d14', 'conversation-d14', 'life-d14', 'turn-d14',
                          'assistant', 'historical assistant content', 2,
                          '2026-08-27T00:00:00.002Z');",
                )
                .unwrap();
        ...
        migration::validate_life_intent_schema(&connection).unwrap();
        migration::validate_experience_episode_schema(&connection).unwrap();
        for table in [
            "life_goal",
            "life_plan",
        ...
        let message_count: i64 = connection
            .query_row(
                "SELECT COUNT(*) FROM conversation_message WHERE conversation_id='conversation-d14'",
                [],
                |row| row.get(0),
            )
            .unwrap();
        ...
        let migration_count: i64 = connection
            .query_row(
                ...
                [migration::LIFE_INTENT_AUTHORITY_SCHEMA_VERSION],
                |row| row.get(0),
            )
            .unwrap();
    ...
    fn d9d3_d_f1_schema_seventeen_startup_upgrades_through_authoritative_coordinator() {
        ...
        // fences, the six Schema20 relationship writer fences, the three
        // Schema21 experience-episode writer fences, and the fifteen Schema22
        // life-intent writer fences are installed alongside the older writer
        // fences.
        let writer_fence_total: i64 = connection
            ...
            .unwrap();
        assert_eq!(writer_fence_total, 120);
