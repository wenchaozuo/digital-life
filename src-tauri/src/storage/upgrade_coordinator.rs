...
mod tests {
    ...
    fn storage_initialize_restored_version_fourteen_validates_then_publishes() {
        ...
        // The schema is at 23, so the version-17 lifecycle validator cannot
        // run alone; the full 87-trigger writer fence plus the exact Schema-19
        ...
        assert_eq!(writer_fence_count(&state.connection), 87);
        assert_eq!(
            take_upgrade_events(),
            vec![
                "mutex",
    ...
    fn d9d3_d_f1_schema_seventeen_startup_upgrades_through_authoritative_coordinator() {
        ...
        let writer_fence_total: i64 = connection
            ...
            .unwrap();
        assert_eq!(writer_fence_total, 87);
